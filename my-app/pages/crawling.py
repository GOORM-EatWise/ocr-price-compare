...
#     try:
#         search_url = f"https://search.danawa.com/dsearch.php?query={search_term}&tab=goods"
...
def render():
    ...
    # Step 2: 용량 선택
        ...
        if st.session_state.original_products:
            ...
            for idx, product in enumerate(st.session_state.original_products):
                option_text = f"{product['prod_name']} - {product['volume']} - {product['price']}"
            ...
            if st.button("선택 완료"):
                st.session_state.selected_product = st.session_state.original_products[selected_idx]
    ...
    # Step 3: 유사 상품 크롤링
        ...
        st.write(f"**선택된 상품**: {selected['prod_name']}")
        st.write(f"**용량**: {selected['volume']}")
        ...
        if st.button("🚀 유사 상품 크롤링 시작"):
            with st.spinner("카테고리 기반 유사 상품을 검색 중입니다..."):
                category = main_product['product_category']
