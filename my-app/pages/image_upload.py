...
def render():
    ...
    if uploaded_file is not None:
        ...
        # 이미지 회전 적용
        rotated_img = img.rotate(-st.session_state.rotation, expand=True)  # 시계방향 회전

        st.image(rotated_img, caption=f'업로드한 이미지 (회전: {st.session_state.rotation}°)', use_container_width=True)
    ...
    with col2:    
        if st.button('이미지 사용'):
            if not uploaded_file:
                ...
                st.session_state.image = np.asarray(rotated_img)
